...
from sklearn.metrics import auc, roc_auc_score, average_precision_score, precision_recall_curve, accuracy_score
...
class Evaluator(object):
    ...
    def run(self, results, cls_name, logger=None):
        ...
        """
        Different datasets generally correspond to different low_sigmas, high_sigmas, betas and gammas, 
        which should be adjusted according to the actual effect.
        """
        ...
        if self.pooling_ks is not None:
            ...
            concentration_ratio = []
            for amap in pr_px:
                contrast = np.mean(np.abs(difference_of_gaussians(amap, low_sigma=0.5, high_sigma=1.5)))
                contrasts.append(contrast)

                amap = torch.tensor(amap)
                max_val, max_idx = torch.max(amap.view(-1), 0)
                h_star, w_star = divmod(max_idx.item(), amap.size(1))
                e_star = max_val.item()
                H, W = amap.size()
                h = torch.arange(H).view(-1, 1).expand(H, W)
                w = torch.arange(W).view(1, -1).expand(H, W)
                h_star_tensor = torch.full((H, W), h_star, dtype=torch.float32)
                w_star_tensor = torch.full((H, W), w_star, dtype=torch.float32)
                d = torch.sqrt((h - h_star_tensor).pow(2) + (w - w_star_tensor).pow(2))
                e = amap
                cost = (e - e_star).pow(2)
                score = torch.mean(d * cost)
                concentration_ratio.append(score)

            contrasts = np.array(contrasts)
            concentration_ratio = np.array(concentration_ratio)

            beta = 250
            gamma = -0.025

            pr_sp_max = pr_sp_max + beta * contrasts
            pr_sp_max = pr_sp_max + gamma * concentration_ratio
            ...
            concentration_ratio = []
            for amap in pr_px:
                contrast = np.mean(np.abs(difference_of_gaussians(amap, low_sigma=0.5, high_sigma=1.0)))
                contrasts.append(contrast)
        ...
        for metric in self.metrics:
            ...
            if metric.startswith('mAUROC_sp_max'):
                ...
                #prediction = [1 if prob > best_threshold else 0 for prob in pr_sp_max]
                #acc = accuracy_score(gt_sp, prediction)
                best_acc = 0
                best_threshold_acc = None
                for threshold in thresholds:
                    prediction = [1 if prob > threshold else 0 for prob in pr_sp_max]
                    acc = accuracy_score(gt_sp, prediction)
                    if acc > best_acc:
                        best_acc = acc
                        best_threshold_acc = threshold
                best_threshold = best_threshold_acc
                metric_results[metric] = best_acc
                #metric_results[metric] = acc
                #print(precisions, recalls, thresholds, best_threshold, pr_sp_max, pr_sp, gt_sp, prediction)
