...
from numba.cuda.testing import skip_if_mvc_libraries_unavailable
...
class SCANS(nn.Module):
	def __init__(self, size=16, dim=2, scan_type='scan', ):
		...
		elif 'CH-Scan' == scan_type:
			num_rings = int(size / 4)
			boundary_indexes = self.get_boundary_indexes(size, dim, num_rings)
			internal_indexes = np.setdiff1d(indexes, boundary_indexes)
			locs_boundary = boundary_indexes
			size_internal = int(size / 2)
			bit = int(np.log2(size_internal))
			internal_indexes_indexes = np.argsort(internal_indexes)
			locs_internal = decode(internal_indexes_indexes, dim, bit)
			locs_internal_flat = self.flat_locs_hilbert(locs_internal, dim, bit)
			locs_internal_flat = internal_indexes[locs_internal_flat]
			locs_flat = np.concatenate((locs_boundary, locs_internal_flat))
		else:
	...
	def get_boundary_indexes(self, size, dim, num_rings):
		boundary_indexes = []
...
class SS2D(nn.Module):
	...
	def forward_core(self, x: torch.Tensor):
		...
		xs = []
		if K >= 2:
			xs.append(self.scans.encode(x.view(B, -1, L)))  # 上顺
		if K >= 4:
			xs.append(self.scans.encode(torch.transpose(x, dim0=2, dim1=3).contiguous().view(B, -1, L)))  # 左逆
		if K >= 8:
			xs.append(self.scans.encode(torch.rot90(x, k=1, dims=(2, 3)).contiguous().view(B, -1, L)))  # 右顺
			xs.append(self.scans.encode(torch.transpose(torch.rot90(x, k=1, dims=(2, 3)), dim0=2, dim1=3).contiguous().view(B, -1, L)))  # 上逆
			#xs.append(self.scans.encode(torch.rot90(torch.rot90(x, k=1, dims=(2, 3)), k=1, dims=(2, 3)).contiguous().view(B, -1, L))) #下顺
			#xs.append(self.scans.encode(torch.rot90(torch.rot90(torch.rot90(x, k=1, dims=(2, 3)), k=1, dims=(2, 3)), k=1,dims=(2, 3)).contiguous().view(B, -1, L))) #左顺
			#xs.append(self.scans.encode(torch.rot90(torch.transpose(x, dim0=2, dim1=3), k=1, dims=(2, 3)).contiguous().view(B, -1, L))) #下逆
			#xs.append(self.scans.encode(torch.rot90(torch.rot90(torch.transpose(x, dim0=2, dim1=3), k=1, dims=(2, 3)), k=1, dims=(2, 3)).contiguous().view(B, -1, L))) #右逆
		...
		if K >= 8:
			...
			#ys.append(torch.rot90(torch.rot90(self.scans.decode(out_y[:, 4]).view(B, -1, W, H), k=3, dims=(2, 3)), k=3, dims=(2, 3)).contiguous().view(B, -1, L))
			#ys.append(torch.rot90(torch.rot90(self.scans.decode(inv_y[:, 4]).view(B, -1, W, H), k=3, dims=(2, 3)), k=3, dims=(2, 3)).contiguous().view(B, -1, L))
			#ys.append(torch.rot90(torch.rot90(torch.rot90(self.scans.decode(out_y[:, 5]).view(B, -1, W, H), k=3, dims=(2, 3)), k=3, dims=(2, 3)), k=3, dims=(2, 3)).contiguous().view(B, -1, L))
			#ys.append(torch.rot90(torch.rot90(torch.rot90(self.scans.decode(inv_y[:, 5]).view(B, -1, W, H), k=3, dims=(2, 3)), k=3, dims=(2, 3)), k=3, dims=(2, 3)).contiguous().view(B, -1, L))
			#ys.append(torch.transpose(torch.rot90(self.scans.decode(out_y[:, 6]).view(B, -1, W, H), k=3, dims=(2, 3)), dim0=2, dim1=3).contiguous().view(B, -1, L))
			#ys.append(torch.transpose(torch.rot90(self.scans.decode(inv_y[:, 6]).view(B, -1, W, H), k=3, dims=(2, 3)), dim0=2, dim1=3).contiguous().view(B, -1, L))
			#ys.append(torch.transpose(torch.rot90(torch.rot90(self.scans.decode(out_y[:, 7]).view(B, -1, W, H), k=3, dims=(2, 3)), k=3, dims=(2, 3)), dim0=2, dim1=3).contiguous().view(B, -1, L))
			#ys.append(torch.transpose(torch.rot90(torch.rot90(self.scans.decode(inv_y[:, 7]).view(B, -1, W, H), k=3, dims=(2, 3)), k=3, dims=(2, 3)), dim0=2, dim1=3).contiguous().view(B, -1, L))
		y = sum(ys)
		return y
...
class ConvBNSSMBlock(nn.Module):
	...
	def forward(self, input: torch.Tensor):
		...
		out_77 = self.conv1a3(self.conv77(self.conv1b3(input_conv))) #official
		out_55 = self.conv1a5(self.conv55(self.conv1b5(input_conv))) #official
		output = torch.cat((out_ssm.permute(0, 3, 1, 2).contiguous(), out_55, out_77), dim=1) #official
...
class MambaUPNet(nn.Module):
	def __init__(self, dims_decoder=[512, 256, 128, 64], depths_decoder=[2, 2, 2, 2], d_state=16, drop_rate=0.,
		...
		self.apply(self._init_weights)
		self.coreset = nn.Parameter(torch.randn(64, 5, 512))
		self.prototype_distance256 = None
		self.vis_conv = nn.Conv2d(64, 1, kernel_size=1)
	...
	def forward(self, x):
		...
		H_l = x
		B, H, W, C = H_l.size()
		H_l = H_l.reshape(B, H * W, C)

		device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
		H_l = H_l.to(device)
		similarity_matrix = torch.zeros(B, H * W, num_prototypes)
		patch_size = int((H * W) ** 0.5)
		...
		min_distance = torch.min(distance, dim=-1)[0]  # [16, 64]

		min_distance = min_distance.reshape(B, 1, H, W)
		prototype_distance256 = F.interpolate(min_distance, size=(256, 256), mode='bilinear', align_corners=False)
		prototype_distance256 = prototype_distance256.squeeze(1)

		self.prototype_distance256 = prototype_distance256
